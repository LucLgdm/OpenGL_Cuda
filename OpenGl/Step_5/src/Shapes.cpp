...
Triangle createTriangle(string name, float size) {
	float vertices[] = {
		-size, -size, 1.0f, 0.0f, 0.0f, // rouge
		size,  -size, 0.0f, 1.0f, 0.0f, // vert
		0.0f,   size, 0.0f, 0.0f, 1.0f  // bleu
	};
...
Square createSquare(string name, float size) {
	float vertices[] = {
		-size, -size, 1.0f, static_cast<float>(rand()) / RAND_MAX, static_cast<float>(rand()) / RAND_MAX, 		// bottom left
		size,  -size, static_cast<float>(rand()) / RAND_MAX,  1.0f,   static_cast<float>(rand()) / RAND_MAX, 	// bottom right
		size,   size, static_cast<float>(rand()) / RAND_MAX,  static_cast<float>(rand()) / RAND_MAX,  1.0f,		// top right
		-size,  size, static_cast<float>(rand()) / RAND_MAX,  0.5f,   static_cast<float>(rand()) / RAND_MAX	// top left
	};
...
Circle createCircle(string name, Color col, float radius, int segments) {
	...
	vertices.push_back(static_cast<float>(rand()) / RAND_MAX); vertices.push_back(static_cast<float>(rand()) / RAND_MAX); vertices.push_back(static_cast<float>(rand()) / RAND_MAX);
